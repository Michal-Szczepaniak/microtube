...
void IVListParser::parseItem(const QJsonObject &item) {
    ...
    QDateTime publishedDateTime = QDateTime::fromTime_t(publishedAt);
    ...
    for (const auto &thumbnail : thumbnails) {
        auto q = thumbnail.toObject()["quality"];
        if (q == QLatin1String("medium")) {
            video->setThumbnailUrl(thumbnail.toObject()["url"].toString());
        } else if (q == QLatin1String("high")) {
            video->setMediumThumbnailUrl(thumbnail.toObject()["url"].toString());
        } else if (q == QLatin1String("sddefault")) {
            video->setLargeThumbnailUrl(thumbnail.toObject()["url"].toString());
        }
    }
